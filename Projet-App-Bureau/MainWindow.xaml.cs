...
namespace Projet_App_Bureau
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            // conBD = new SqlConnection(@"Data Source=MAME\SQLEXPRESS;Initial Catalog=GestionStagiaire;Integrated Security=True");
            // Clé de connexion: pour Lamia
            conBD = new SqlConnection(@"Data Source=DESKTOP-EA7s6BR;Initial Catalog=GestionStagiaire;Integrated Security=True");
        ...
        private void Charger_Liste_stagiaire()
        {
            ...
            string id = "ID Stagiaire";
            string nom = "Nom Stagiaire";
            string prenom = "Prénom Stagiaire";
            string sexe= "Sexe Stagiaire";
            string date_naissance = "Date de naissance";
            string programme = "ID Programme";
        ...
        private void dgProgramme_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            {
                txtNumeroProgramme.Text = ligne_selectionnee["ID Programme"].ToString();
                txtNomProgramme.Text = ligne_selectionnee["Nom Programme"].ToString();
                txtDureeProgramme.Text = ligne_selectionnee["Durée Programme"].ToString();
            }
        ...
        {
            string nom = "Nom Programme";
            string id = "ID Programme";
            string duree = "Durée Programme";
        ...
        private void dgStagiaire_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (ligne_selectionnee != null)
            {
                ...
                textBoxNumero.Text = ligne_selectionnee["ID Stagiaire"].ToString();

                // 2-Fill le textBox "Nom"
                textBoxNom.Text = ligne_selectionnee["Nom Stagiaire"].ToString();

                // 3-Fill le textBox "Prénom"
                textBoxPrenom.Text = ligne_selectionnee["Prénom Stagiaire"].ToString();

                // ******************4-Fill le datePicker "Date de naissance"*********************
                // Source: https://stackoverflow.com/questions/20743121/how-can-i-get-sql-result-into-a-string-variable

                // Query qui cherche la date de naissance du stagiaire
                string query_getDateNaissance = "SELECT date_naissance FROM stagiaire WHERE id_stagiaire = " + ligne_selectionnee["ID Stagiaire"].ToString() + ";";
                ...
                string query_getSexe = "SELECT sexe_stagiaire FROM stagiaire WHERE id_stagiaire = " + ligne_selectionnee["ID Stagiaire"].ToString() + ";";
                ...
                if (query_result_getSexe == "Masculin")
                {
                    radioButton_masculin.IsChecked = true;
                }
                else if (query_result_getSexe == "Féminin")
                {
                    radioButton_feminin.IsChecked = true;
                }
                else if (query_result_getSexe == "Autre")
                {
                ...
                string query_getIdProgramme = "SELECT id_programme FROM stagiaire WHERE id_stagiaire = " + ligne_selectionnee["ID Stagiaire"].ToString() + ";";
        ...
        private void buttonModifierStagiaire_Click(object sender, RoutedEventArgs e)
        {
            ...
            // #1 Numéro
            // Si le textBox numéro du stagiaire n'est pas vide ou il n'est pas un nombre positif
            /*if (!int.TryParse(textBoxNumero.Text, out int numeroStagiaire) || int.Parse(textBoxNumero.Text) < 0 || textBoxNumero.Text == "")
            {
                ...
                MessageBox.Show("SVP veuillez entrez un numéro de stagiaire valide!");
            }*/
            ...
            if (textBoxNom.Text == "" || int.TryParse(textBoxNom.Text, out int n))
            {
                // Mettre la bordure du textbox en rouge pour avertir l'utilisateur
                txtNomProgramme.BorderBrush = Brushes.Red;
                MessageBox.Show("SVP veuillez entrez un nom de stagiaire valide !");
            }
            // #3 Prénom
            // Si le textBox prénom du stagiaire est vide ou s'il est numerique.
            else if (textBoxPrenom.Text == "" || int.TryParse(textBoxPrenom.Text, out int nn))
            {
                // Mettre la bordure du textbox en rouge pour avertir l'utilisateur
                txtNomProgramme.BorderBrush = Brushes.Red;
                MessageBox.Show("SVP veuillez entrez un prénom de stagiaire valide !");
            }

            // On ne vérifie pas si le DatePicker, le radioButton ou le comboBox sont vides
            // car ils ont déjà une valeur sélectionné et ne peuvent pas devenir vide par la suite
        ...
        private void cbxConsulter_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Veuillez selectionner un numéro !");
            }
        }

        private void txtDureeProgramme_TextChanged(object sender, TextChangedEventArgs e)
        {
